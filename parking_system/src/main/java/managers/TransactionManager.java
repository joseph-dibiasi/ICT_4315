...
public class TransactionManager {
    ...
    /**
     * Handle a ParkingEvent (enter/exit) and register parking charges
     * accordingly.
     */
    public ParkingCharge park(ParkingEvent event) {
        if (event.getType() == EventType.ENTRY) {
            return this.park(event.getDateTime(), event.getLot(), event.getCar());
        } else {
            // convert LocalDateTime to Instant using system default zone
            return this.leave(event.getDateTime().atZone(ZoneId.systemDefault()).toInstant(), event.getLot(), event.getCar());
        }
    }
